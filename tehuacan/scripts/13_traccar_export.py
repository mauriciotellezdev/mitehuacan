...
def main():
    ...
    ap.add_argument("--device", required=True, help="device identifier in Traccar (e.g. mauricio-1)")
    ...
    if not pos:
        raise SystemExit("no positions in that window — check times (local) and that the phone flushed its buffer")
    pos.sort(key=lambda p: p["fixTime"])
    coords = [[round(p["longitude"], 6), round(p["latitude"], 6)] for p in pos]
