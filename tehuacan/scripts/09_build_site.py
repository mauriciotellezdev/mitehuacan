...
.cards{display:grid;grid-template-columns:1fr;gap:14px;margin:26px 0 8px}
@media(min-width:640px){.cards{grid-template-columns:1fr 1fr}}
.card{position:relative;display:block;padding:18px;border:1px solid var(--line);border-radius:14px;
...
.badge{position:absolute;top:14px;right:14px;font-size:11px;font-weight:600;
 padding:3px 9px;border-radius:99px;background:var(--chip);color:var(--ink2);border:1px solid var(--line)}
a.card.live{grid-column:1/-1;border-color:var(--accent);background:var(--bg)}
a.card.live .badge{background:var(--accent);color:#fff;border-color:var(--accent)}
