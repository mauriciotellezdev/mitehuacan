...
export async function onRequestGet({ env }) {
  const { results } = await env.DB.prepare(
    "SELECT slug, first_run, last_run, headway_min, notes FROM combi_lines " +
    "WHERE first_run IS NOT NULL OR last_run IS NOT NULL OR headway_min IS NOT NULL").all();
  const schedules = {};
  for (const r of results) {
    schedules[r.slug] = { first_run: r.first_run, last_run: r.last_run,
                          headway_min: r.headway_min, notes: r.notes };
  }
